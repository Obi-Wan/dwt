...
const static char error_msg_num_args[] = "You need to provide an array, level indication and threshold";
...
void mexFunction(int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
  ...
  if (mxIsCell(prhs[0]))
  {
    ...
    if (type == mxDOUBLE_CLASS)
    {
      ...
      if (!dwt_haar_soft_threshold(arrays, dims, levels, thr, error_msg))
      {
    ...
    else
    {
      ...
      if (!dwt_haar_soft_threshold(arrays, dims, levels, (float)thr, error_msg))
      {
        mexErrMsgIdAndTxt(error_id_internal, error_msg.c_str());
        return;
      }
  ...
  else
  {
    if (type == mxDOUBLE_CLASS)
    {
      if (!dwt_haar_soft_threshold((double *)mxGetData(prhs[0]), dims, levels, thr, error_msg))
      {
        mexErrMsgIdAndTxt(error_id_internal, error_msg.c_str());
        return;
      }
    ...
    {
      if (!dwt_haar_soft_threshold((float *)mxGetData(prhs[0]), dims, levels, (float)thr, error_msg))
      {
        mexErrMsgIdAndTxt(error_id_internal, error_msg.c_str());
        return;
      }
